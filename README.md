...
1. **从 WordPress.org 安装**：前往 [Wordnest 插件页](https://wordpress.org/plugins/wordnest/) 安装或下载。
2. **从 GitHub 下载**：也可以从 [Releases](https://github.com/Simon-xuan/Wordnest/releases) 页面获取最新的 `Wordnest.zip`。
3. **上传**：登录 WordPress 后台 → **插件** → **添加新插件** → **上传插件**，选择 `Wordnest.zip`。
4. **激活**：安装完成后点击 **激活插件**，左侧出现「词汇表」菜单，插件设置位于 **设置 → 轻量级词汇表**。
